...
def model(X,Mb,Rb,Md,Rd,Mh,Rh,xcm,ycm,theta,incl,choice):
...
def log_likelyhood(par,X,data,data_err,valuto_data,choice):

    Mb,Rb,Md,Rd,Mh,Rh,xcm,ycm,theta,incl = par

    if choice == 'fotometria':
        rho_m = model(X,Mb,Rb,Md,Rd,Mh,Rh,xcm,ycm,theta,incl,choice)
        valuto = valuto_data
        mod = np.log10(rho_m[valuto])
    elif choice == 'cinematica':
        v_m,sigma_m = model(X,Mb,Rb,Md,Rd,Mh,Rh,xcm,ycm,theta,incl,choice)
        valuto = np.concatenate((valuto_data,valuto_data)) 
        mod = np.concatenate((v_m[valuto_data],sigma_m[valuto_data]))
    elif choice == 'fotometria + cinematica':
        rho_m,v_m,sigma_m = model(X,Mb,Rb,Md,Rd,Mh,Rh,xcm,ycm,theta,incl,choice)
